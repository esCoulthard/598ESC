...
def compute_metrics_repeated(G, trials=100):
    trial_sps = []
    trial_bet = []
    trial_clust = []
    all_weights = {}
...
def plot_metric_merged(metric_name, metrics_dict, title_prefix="Control", 
    ...
    for hemi in ["Left Hemisphere", "Right Hemisphere"]:
        if individual_metrics is not None and hemi in individual_metrics:
            marker = "o" if hemi == "Left Hemisphere" else "^"
            indiv_values = individual_metrics[hemi].get(metric_name, [])
            n = len(indiv_values)
            if n > 1:
                jitter = np.linspace(jitter_range[0], jitter_range[1], n)
            else:
                jitter = [0]
            for j, val in enumerate(indiv_values):
                ax.scatter(x_indiv_fixed + jitter[j], val, color=palette[j % len(palette)], s=80, 
                           zorder=10, marker=marker,
                           label=f"Mouse {individual_labels[hemi][j]}" if j < len(individual_labels[hemi]) else f"Mouse {j}")

    ax.set_xticks(list(base_positions.values()))
    ax.tick_params(axis='y', labelsize=15)
    ax.set_xticklabels(methods, rotation=15, fontsize=15)
    ax.set_title(f"{title_prefix} - {metric_name}", fontsize=15)
    ...
    for h, l in zip(handles, labels_):
        if l not in unique:
            unique[l] = h
    ax.legend(list(unique.values()), list(unique.keys()), loc='upper left', bbox_to_anchor=(1,1))
    ax.text(x_indiv_fixed+0.6, ax.get_ylim()[0] - 0.01*(ax.get_ylim()[1]-ax.get_ylim()[0]),
            "Individual Mice", ha='center', va='top', fontsize=15, rotation=15)
...
def print_and_plot_combined_metrics_merged(file1, file2, control_individual_files, iso_individual_files, 
    ...
    control_metrics = analyze_graph_metrics_both_cases(file1)
    iso_metrics = analyze_graph_metrics_both_cases(file2)
    control_G_left, control_G_right = compute_average_network_with_zeros(control_individual_files)
    iso_G_left, iso_G_right = compute_average_network_with_zeros(iso_individual_files)
    control_combined = {
        "Left Hemisphere": compute_metrics(control_G_left),
        "Right Hemisphere": compute_metrics(control_G_right)
    }
    iso_combined = {
        "Left Hemisphere": compute_metrics(iso_G_left),
        "Right Hemisphere": compute_metrics(iso_G_right)
    }
    control_metrics["Combined Average"] = control_combined
    iso_metrics["Combined Average"] = iso_combined

    print("--- Control (Original Weights) ---")
    print(f"Left Hemisphere:  {control_metrics['Original Weights']['Left Hemisphere']}")
    print(f"Right Hemisphere: {control_metrics['Original Weights']['Right Hemisphere']}\n")
    print("--- Isoflurane (Original Weights) ---")
    ...
    for metric in metrics_to_plot:
        ...
        plot_metric_merged(metric, control_metrics, title_prefix="Control", 
                           individual_metrics=control_individual, individual_labels=control_labels, 
                           legend_outside=True, ylim=ylims)
    for metric in metrics_to_plot:
        ylims = ylim_dict.get(metric) if ylim_dict and metric in ylim_dict else None
        plot_metric_merged(metric, iso_metrics, title_prefix="Isoflurane 1%", 
