...
# ---------------------------
# This computes the cumulative distribution function for the Ketamine condition, and downloads/displays it for the in/out degrees, along with the left/right hemispheres, giving 4 outputs.
# ---------------------------
def compute_shuffled_degrees(csv_file, hemisphere="left", trials=1000, 
    ...
    average_shuffled_in = None
    average_shuffled_out = None
...
def plot_combined_cdf(control_csv, iso_csv, hemisphere, trials, bin_size, xlim, ylim, condition,
    ...
    plt.step(x_iso, cdf_iso, where='post', linewidth=2, color="red", label="Ket. 10mg/kg")
    
    # Process and plot high dose condition.
    if extra_iso2_file is not None:
        ...
        plt.step(x_extra, cdf_extra, where='post', linewidth=2, color="magenta", label="Ket. 100mg/kg")
